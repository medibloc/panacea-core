...
run_check() {
	root=$1
	output=$2
	fake_go_version=${3:-go1.23.12}
	fake_goversion=${4:-go1.23.12}
	fake_goroot=${5:-$root}
...
matching_root=$(make_fixture matching 'compile version go1.23.12')
run_check "$matching_root" "$fixture_root/matching.out"
grep -q 'E2E Go toolchain: go1.23.12' "$fixture_root/matching.out"
...
executable_mismatch_root=$(make_fixture executable-mismatch 'compile version go1.23.12')
if run_check "$executable_mismatch_root" "$fixture_root/executable-mismatch.out" go1.25.7 go1.23.12; then
...
goversion_mismatch_root=$(make_fixture goversion-mismatch 'compile version go1.23.12')
if run_check "$goversion_mismatch_root" "$fixture_root/goversion-mismatch.out" go1.23.12 go1.25.7; then
...
split_root=$(make_fixture split-root 'compile version go1.23.12')
split_toolchain_root=$(make_fixture split-toolchain 'compile version go1.23.12')
if run_check \
	"$split_root" \
