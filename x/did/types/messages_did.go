...
func (msg *MsgCreateDID) ValidateBasic() error {
	if !ValidDID(msg.DID) {
		return Error(ErrInvalidDID, msg.DID)
	}
	if !msg.Document.Valid() {
		return Error(ErrInvalidDIDDocument, msg.Document)
	}
	if msg.Signature == nil || len(msg.Signature) == 0 {
		return Error(ErrInvalidSignature, msg.Signature)
	}
	...
	if addr.Empty() {
		return Error(sdkerrors.ErrInvalidAddress, addr.String())
	}
	return nil
}
