...
func GetCmdCreateDID(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			privKey, err := keys.GeneratePrivKey(mnemonic, bip39Passphrase)
			if err != nil {
				return err
			}
...
// It returns empty strings in the non-interactive mode, so that they can be auto-generated by keys.GeneratePrivKey.
...
func savePrivKeyToKeyStore(keyID types.KeyID, privKey secp256k1.PrivKeySecp256k1, reader *bufio.Reader) error {
	...
	ks, err := keys.NewKeyStore(keystoreBaseDir())
	if err != nil {
		return err
	}
