...
func TestCaptureGoToolchainEvidence(t *testing.T) {
	...
	executable := filepath.Join(string(filepath.Separator), "opt", "go1.26.5", "bin", "go")
	responses := map[string][]byte{
		"version": []byte("go version go1.26.5 linux/amd64\n"),
		"env": []byte(`{
  "GOVERSION": "go1.26.5",
  "GOROOT": "/opt/go1.26.5",
  "GOTOOLDIR": "/opt/go1.26.5/pkg/tool/linux_amd64",
  "GOBIN": "/work/bin",
  "GOOS": "linux",
  "GOARCH": "amd64"
}`),
		"compiler": []byte("compile version go1.26.5\n"),
	}
	...
	require.Equal(t, "go version go1.26.5 linux/amd64", evidence.GoVersionOutput)
	require.Equal(t, "go1.26.5", evidence.GOVersion)
	require.Equal(t, "/opt/go1.26.5", evidence.GORoot)
	require.Equal(t, "/opt/go1.26.5/pkg/tool/linux_amd64", evidence.GoToolDir)
	require.Equal(t, "compile version go1.26.5", evidence.CompilerVersion)
...
func TestCaptureGoToolchainEvidenceReportsProbeFailures(t *testing.T) {
	...
	base := goToolchainProbe{
		...
		run: func(_ context.Context, _ string, arguments ...string) ([]byte, error) {
			...
			case "env":
				return []byte(`{"GOVERSION":"go1.26.5","GOROOT":"/opt/go","GOTOOLDIR":"/opt/go/pkg/tool/linux_amd64","GOOS":"linux","GOARCH":"amd64"}`), nil
			case "tool":
				return []byte("compile version go1.26.5"), nil
			default:
...
func validGoToolchainEvidence() GoToolchainEvidence {
	return GoToolchainEvidence{
		...
		GoVersionOutput:      "go version go1.26.5 linux/amd64",
		GOVersion:            "go1.26.5",
		GORoot:               "/opt/go",
		GoToolDir:            "/opt/go/pkg/tool/linux_amd64",
		CompilerVersion:      "compile version go1.26.5",
		GoOS:                 "linux",
		GoArch:               "amd64",
		HostOS:               "linux",
		HostArch:             "amd64",
