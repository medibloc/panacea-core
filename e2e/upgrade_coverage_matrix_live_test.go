...
func buildConnectedUpgradeCoverageMatrix(input connectedUpgradeCoverageInput) harness.UpgradeCoverageMatrix {
	...
	rows := []harness.UpgradeCoverageRow{
		passedRow(
			...
			[]string{"upgrade_deep_helpers_test.go", "upgrade_compatible_signed_tx_test.go", "upgrade_test.go"},
			[5][]string{
		...
		passedRow(
			...
			harness.UpgradeCoveragePriorityP0,
			[]string{"signing-info:" + input.Staking.ValidatorConsensusAddr},
			[]string{"upgrade_staking_matrix_test.go"},
			[5][]string{
				{"upgrade/staking/preparation.json"},
				{"upgrade/staking/checkpoints/pre-upgrade-checkpoint.json"},
		...
		passedRow(
			...
			harness.UpgradeCoveragePriorityP0,
			[]string{"did:" + input.Preserved.DID.Updated.DID, "did:" + input.Preserved.DID.Deactivated.DID},
			[]string{"upgrade_did_matrix_test.go"},
			[5][]string{
		...
		passedRow(
			...
			harness.UpgradeCoveragePriorityP0,
			[]string{"aol-topic:" + input.Preserved.AOLFixture.TopicName},
			[]string{"internal/harness/upgrade_aol_test.go"},
			[5][]string{
