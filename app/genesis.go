...
// PanaceaAppGenState creates the core parameters for genesis initialization for Panacea
...
func NewDefaultGenesisState() GenesisState {
	...
	minter := mint.InitialMinter(sdk.NewDecWithPrec(5, 2)) // initial inflation rate of 5.00%
	mintGenState.Minter = minter
	mintGenState.Params.MintDenom = assets.MicroMedDenom
	mintGenState.Params.InflationRateChange = sdk.NewDecWithPrec(2, 2) // maximum annual inflation change rate is 2.00%
	mintGenState.Params.InflationMin = sdk.NewDecWithPrec(4, 2)        // min inflation rate is 4.00%
	mintGenState.Params.InflationMax = sdk.NewDecWithPrec(6, 2)        // max inflation rate is 6.00%
	...
	govGenState := gov.DefaultGenesisState()
	minDepositTokens := sdk.TokensFromTendermintPower(100000) // 100,000 MED
	...
	slashingGenState := slashing.DefaultGenesisState()
	...
	slashingGenState.Params.MinSignedPerWindow = sdk.NewDecWithPrec(5, 2) // 5%
...
// MediValidateGenesisState ensures that the genesis state obeys the expected invariants
...
func PanaceaAppGenStateJSON(cdc *codec.Codec, genDoc tmtypes.GenesisDoc, appGenTxs []json.RawMessage) (
	...
	genesisState, err := PanaceaAppGenState(cdc, genDoc, appGenTxs)
	if err != nil {
		return nil, err
	}
...
func NewDefaultGenesisAccount(addr sdk.AccAddress) GenesisAccount {
	...
	coins := sdk.Coins{
		sdk.NewCoin(assets.MicroMedDenom, sdk.NewInt(1000).MulRaw(assets.MicroUnit)),
	}
