...
func TestnetFilesCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a panacead testnet",
...
Example:
	panacead testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "panacead",
		"Home directory of the node's daemon configuration",
	)
	cmd.Flags().String(flagNodeCliHome, "panaceacli",
	...
	cmd.Flags().String(
		server.FlagMinGasPrices, fmt.Sprintf("1%s", assets.MicroMedDenom),
		"Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 1nmed)",
	)
...
func initTestnet(config *tmconfig.Config, cdc *codec.Codec) error {
	...
	panaceaConfig := srvconfig.DefaultConfig()
	panaceaConfig.MinGasPrices = viper.GetString(server.FlagMinGasPrices)
	...
	for i := 0; i < numValidators; i++ {
		...
		coin := sdk.TokensFromTendermintPower(500) // TODO @ggomma check power - coin relation
		accs = append(accs, app.GenesisAccount{
			Address: addr,
			Coins: sdk.Coins{
				sdk.NewCoin(assets.MicroMedDenom, coin),
			},
		})
		...
		msg := staking.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(assets.MicroMedDenom, valTokens),
		...
		panaceaConfigFilePath := filepath.Join(nodeDir, "config/panacead.toml")
		srvconfig.WriteConfigFile(panaceaConfigFilePath, panaceaConfig)
	}
