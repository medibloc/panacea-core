...
var (
	defaultTokens                  = sdk.TokensFromTendermintPower(1000000) // 100 * (10 ** 6)
	defaultAmount                  = defaultTokens.String() + assets.MicroMedDenom
	defaultCommissionRate          = "0.1"
	defaultCommissionMaxRate       = "0.2"
	defaultCommissionMaxChangeRate = "0.01"
	defaultMinSelfDelegation       = "1000000"
)

// GenTxCmd builds the panacead gentx command.
// nolint: errcheck
func GenTxCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		Args:  cobra.NoArgs,
		// TODO @ggomma check panacead tx create-validator command. It does not work
		Long: fmt.Sprintf(`This command is an alias of the 'panacead tx create-validator' command'.
...
`, defaultAmount, defaultCommissionRate, defaultCommissionMaxRate, defaultCommissionMaxChangeRate, defaultMinSelfDelegation),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err = app.PanaceaValidateGenesisState(genesisState); err != nil {
				return err
			}
			...
			if info.GetType() == kbkeys.TypeOffline || info.GetType() == kbkeys.TypeMulti {
				fmt.Println("Offline key passed in. Use `panaceacli tx sign` command to sign:")
