...
func DecryptDataCmd(defaultNodeHome string) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "decrypt-data [input-file-path] [key-name] [encrypted-combined-key]",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			encryptedCombinedKey, err := base64.StdEncoding.DecodeString(args[2])
			if err != nil {
				return err
			}
			...
			decryptedData, err := decrypt(clientCtx, args[1], encryptedCombinedKey, oraclePubKey, encryptedData)
			if err != nil {
				return err
			}
