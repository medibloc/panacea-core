...
Example:
	...
	cmd.Flags().String(
		server.FlagMinGasPrices, fmt.Sprintf("0.000006%s", assets.MicroMedDenom),
...
func InitTestnet(cmd *cobra.Command, config *tmconfig.Config, cdc *codec.Codec,
	...
	panaceaConfig.MinGasPrices = minGasPrices
	...
	for i := 0; i < numValidators; i++ {
		...
		accs = append(accs, genaccounts.GenesisAccount{
			...
			Coins: sdk.Coins{
				...
				sdk.NewCoin(assets.MicroMedDenom, accStakingTokens),
			},
		})
		...
		// TODO: Rename config file to server.toml as it's not particular to Panacea
